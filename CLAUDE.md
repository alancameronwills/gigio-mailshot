...
- **HTML rendering** — `eventsToHtml()` builds the actual email HTML (inline `<style>`, per-event blocks). A `booking` flag (non-empty `bookinglink` meta) shows a badge labelled with `booklabel` (or "Booking essential" as a default). Image width is capped at 350px both via the `.events img` CSS rule *and* a `style` attribute directly on the `<img>` tag — the CSS-only rule alone wasn't enough because some recipients' email clients strip/ignore `<style>` blocks, so the constraint needs to be inline on the tag too. Keep both when touching this.
